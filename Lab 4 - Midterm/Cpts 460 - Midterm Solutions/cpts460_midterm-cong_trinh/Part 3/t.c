...
void IRQ_handler()
{
  ...
  int flag, proc_timers = total_timers;
  //flag = timer_handler(0);
  for (int i = 1; i < proc_timers; i++)
  {
...
int kfork(int run_f)
{
  ...
  for (i = 1; i < 15; i++)
    p->kstack[SSIZE - i] = 0;
  p->kstack[SSIZE - 1] = (int)run_f; // in dec reg=address ORDER !!!
...
int do_timer()
{
  ...
  timer_start(total_timers, running->pid, sec);
  ksleep(running);
  printList("sleepList", sleepList);
}
